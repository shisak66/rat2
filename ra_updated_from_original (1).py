...
def notify_user_owner(chat_id, fields):
    ...
    send_msg(chat_id, text, reply_markup=reply_markup)
    send_msg(OWNER_IDS, text, reply_markup=reply_markup)
...
# ===== ADDED: extract device id from replied message =====
...
# ===== ADDED: Firebase optimized device query (no full DB download) =====
def firebase_query_by_device(base_url, device_id, field="deviceId"):
    base = base_url.rstrip("/")
    if not base.endswith(".json"):
        base += "/.json"

    url = f'{base}?orderBy="{field}"&equalTo="{device_id}"'
    try:
        ...
        print("firebase_query_by_device error:", e)
        return []
